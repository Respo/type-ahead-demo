...
(task-options!
  pom {:project     'respo/type-ahead-demo
       :version     +version+
       :description "Workflow"
       :url         "https://github.com/Respo/type-ahead-demo"
       :scm         {:url "https://github.com/Respo/type-ahead-demo"}
...
(deftask dev! []
  ...
  (comp
    ...
    (reload :on-jsload 'type-ahead-demo.core/on-jsload
            :cljs-asset-path ".")
    (cljs)
    (target)))
...
(deftask rsync []
  (with-pre-wrap fileset
    (sh "rsync" "-r" "target/" "tiye:repo/Respo/type-ahead-demo" "--exclude" "main.out" "--delete")
    fileset))
...
(deftask watch-test []
  ...
  (comp
    (watch)
    (test :namespaces '#{type-ahead-demo.test})))
